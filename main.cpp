...
bool powerConsumption(std::vector<std::vector<float>>);

int main()
{
	std::vector<std::vector<float>> dataModel = 
    {
    ...
    bool robotMode = powerConsumption(dataModel);

    if (robotMode)
        std::cout << "Robot can finish the path";
    else
        std::cout << "Robot cannot finish the path";

}

bool powerConsumption(std::vector<std::vector<float>> datamodel)
{
    ...
    // assuming no drag, acceleration and gradient since robot is operated at low speed
    float force;
    force = mu * mass * g; // watt

    // assuming constant velocity 9 km/hr and no efficiency loss (for simplification)
    float vel = 2.5;
    float power = force * vel;

    // assumed battery spec
    float volt = 14.4; //volt
    int current = 5.2; // ah
    float batp = volt * current; //wh

    // assumed 100% initial charge (for simplification)
    float totalTime = batp / power;
    float estDistance = vel * totalTime * 3600 ; // m
    ...
    {
        tempd = std::sqrt(pow((datamodel[i + 1][0] - datamodel[i][0]), 2) + pow((datamodel[i + 1][1] - datamodel[i][1]), 2));
        distance = distance + tempd; // m
    }

    distance = distance * pow(10, 3);

    std::cout << estDistance << "\n" << distance;
    if (estDistance < distance)
        return false;
    else
        return true;
}